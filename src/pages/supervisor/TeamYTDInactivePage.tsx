/* Varchaz — Supervisor Team YTD Activity Matrix Page */
...
export default function TeamYTDInactivePage() {
  ...
  async function load() {
    ...
    try {
      ...
      const ytdMonths = getYTDMonths(appUser.financialYear || 'apr-mar');
      const periodStart = ytdMonths.length > 0 ? `${ytdMonths[0]}-01` : '2000-01-01';
  ...
  const handleSendEmail = async () => {
    ...
    try {
      const title = `Team YTD Activity Matrix — ${fyLabel}`;
  ...
  return (
    <div className="dashboard-page" id="team-ytd-inactive-page">
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 'var(--v-space-4)', flexWrap: 'wrap', gap: 'var(--v-space-4)' }}>
        <PageHeader
          title="Team YTD Activity Matrix"
          subtitle={`Active vs Inactive products per team member in ${fyLabel}`}
        />
          ...
          <div className="modal-content" style={{ maxWidth: 480, width: '90%' }}>
            ...
            <div className="modal-body" style={{ display: 'flex', flexDirection: 'column', gap: 'var(--v-space-4)' }}>
              <p style={{ fontSize: 'var(--v-text-xs)', color: 'var(--v-text-secondary)', marginBottom: 'var(--v-space-1)' }}>
                This will send the cross-tabulated HTML matrix of <strong>Team YTD Activity</strong> to the email below.
              </p>
